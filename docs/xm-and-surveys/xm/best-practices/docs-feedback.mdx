...
- Create a [Formbricks Cloud](https://app.merdekasurvey.com/auth/signup) account.
...
- Then, create a survey using the template [Docs Feedback](https://merdekasurvey.com/survey-templates/docs-feedback):
            ...
            <div className="ml-4 inline-flex space-x-3">
              {["Yes 👍", " No 👎"].map((option) => ( // Popup Trigger is a button as well. This is a workaround to open the same form but send a different response to the API
                <PopoverTrigger
                  ...
                  {option} // "Yes 👍" or "No 👎" - they have to be identical with the choices in the survey on app.merdekasurvey.com for it to work (!)
                </PopoverTrigger>
              ))}
            </div>
            <PopoverContent className="border-slate-300 bg-white dark:border-slate-500 dark:bg-slate-700">
...
export const handleFeedbackSubmit = async (YesNo, pageUrl) => {
  const response_data = {
    data: {
      isHelpful: YesNo, // the "Yes 👍" or "No 👎" response. Remember: They have to be identical with the choices in the survey on app.merdekasurvey.com for it to work.
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_FORMBRICKS_COM_API_HOST}/api/v1/client/environments/${process.env.NEXT_PUBLIC_FORMBRICKS_COM_ENVIRONMENT_ID}/responses`, // For testing, replace this with the API host and environemnt ID of your Development environment on app.merdekasurvey.com
  };
      {
        method: "POST",
        headers: {
...
- Environment ID (1) of the development environment on app.merdekasurvey.com
...
```tsx Replace the ID and API accordingly
  ...
  try {
    const res = await fetch(
			// Note that we also updated the API host to 'https://app.merdekasurvey.com/'
      `https:app.merdekasurvey.com/api/v1/client/environments/clgwcwp4z000lpf0hur7uxbuv/responses`,
  };
```
...
```tsx Replace the ID and API here as well
  try {
    const res = await fetch(
		// Note that we also updated the API host to 'https://app.merdekasurvey.com/'
      `https:app.merdekasurvey.com/api/v1/client/environments/clgwcwp4z000lpf0hur7uxbuv/responses/${responseId}`, // Note that we also updated the API host to 'https://app.merdekasurvey.com/'
}
```
